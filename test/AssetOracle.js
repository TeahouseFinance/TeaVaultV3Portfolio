...
async function deployAssetOracle() {
    ...
    return { owner, user, assetOracle, token0, token1, token2, token3 }
}

describe("AssetOracle", function () {
    it("Should set the correct decimals", async function () {
        const { assetOracle } = await helpers.loadFixture(deployAssetOracle);

        expect(await assetOracle.decimals()).to.equal(18n);
    });

    it("Should set the correct base token", async function () {
        const { assetOracle, token0 } = await helpers.loadFixture(deployAssetOracle);
    ...
    it("Should be able to enable oracle from owner", async function() {
        const { owner, assetOracle, token0, token1 } = await helpers.loadFixture(deployAssetOracle);
        ...
        expect(await assetOracle.isOracleEnabled(token0.target)).to.equal(true);
        expect(await assetOracle.isOracleEnabled(token1.target)).to.equal(true);
    });

    it("Should not be able to enable oracle from non-owner", async function() {
        const { assetOracle, user, token0, token1 } = await helpers.loadFixture(deployAssetOracle);
       
        const factory = await ethers.getContractAt(uniswapFactoryABI, testFactory);
        const pool = await factory.getPool(token0.target, token1.target, testFeeTier);
        await expect(assetOracle.connect(user).enableOracle(token1.target, [pool], [300])).to.be.revertedWith("Ownable: caller is not the owner");
    });

    it("Should be able to get token twap", async function() {
        const { assetOracle, token0, token1 } = await helpers.loadFixture(deployAssetOracle);
       
        const factory = await ethers.getContractAt(uniswapFactoryABI, testFactory);
        const pool = await factory.getPool(token0.target, token1.target, testFeeTier);
        await assetOracle.enableOracle(token1.target, [pool], [300]);

        const value = await assetOracle.getTwap(token1.target);
        expect(value).to.equal(1917207001122492887082n);    // this value depends on actual market price at the time of the block
    });

    it("Should be able to get token twap using multiple pool price source", async function() {
        // token0: USDC, token1: WETH, token3: WBTC
        const { assetOracle, token0, token1, token3 } = await helpers.loadFixture(deployAssetOracle);

        const factory = await ethers.getContractAt(uniswapFactoryABI, testFactory);
        // WBTC/WETH
        const pool1 = await factory.getPool(token3.target, token1.target, testFeeTier);
        // WETH/USDC
        const pool2 = await factory.getPool(token0.target, token1.target, testFeeTier);

        await assetOracle.enableOracle(token3.target, [pool1, pool2], [300, 300]);
        const value = await assetOracle.getTwap(token3.target);
        expect(value).to.equal(30070048383433408620358n);
    });

    it("Should be able to get multiple token twap", async function() {
        const { assetOracle, token0, token1, token2, token3 } = await helpers.loadFixture(deployAssetOracle);
        ...
        const value1 = await assetOracle.getTwap(token1.target);
        const value2 = await assetOracle.getTwap(token2.target);
        const value3 = await assetOracle.getTwap(token3.target);
        expect(await assetOracle.getTwap(token0.target)).to.equal(ethers.parseUnits("1", await assetOracle.decimals()));
        expect(value2).to.equal(1000900360084012601n);
        expect(value3).to.equal(30055017868696708191670n);
        expect(await assetOracle.getBatchTwap([ token1.target, token2.target ]))
    ...
    it("Should be able to get token value", async function() {
        ...
        const factory = await ethers.getContractAt(uniswapFactoryABI, testFactory);
        const pool1 = await factory.getPool(token0.target, token1.target, testFeeTier);
        const pool2 = await factory.getPool(token3.target, token0.target, testFeeTier);
        await assetOracle.enableOracle(token1.target, [pool1], [300]);
        await assetOracle.enableOracle(token3.target, [pool2], [300]);
        
        expect(await assetOracle.getValue(token1.target, ethers.parseUnits("10", await token1.decimals())))
        .to.equal(19172070011224928870820n); // this value depends on actual market price at the time of the block
        expect(await assetOracle.getValue(token3.target, ethers.parseUnits("10", await token3.decimals())))
    ...
    it("Should not be able to get token twap without enabling oracle", async function() {
        const { assetOracle, token1 } = await helpers.loadFixture(deployAssetOracle);

        await expect(assetOracle.getTwap(token1.target)).to.be.revertedWithCustomError(assetOracle, "AssetNotEnabled");
    });

    it("Should not be able to get token value without enabling oracle", async function() {
        const { assetOracle, token1 } = await helpers.loadFixture(deployAssetOracle);
       
        await expect(assetOracle.getValue(token1.target, ethers.parseUnits("10", await token1.decimals())))
        .to.be.revertedWithCustomError(assetOracle, "AssetNotEnabled");
    });    
});
